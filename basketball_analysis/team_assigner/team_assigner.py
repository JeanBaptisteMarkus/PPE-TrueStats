...
class TeamAssigner:
    ...
    def get_player_teams_across_frames(self, video_frames, player_tracks, read_from_stub=False, stub_path=None):
        ...
        track_votes = defaultdict(lambda: defaultdict(float))  # track_id -> {team: somme_confiances}
        track_counts = defaultdict(lambda: defaultdict(int))   # track_id -> {team: nb_detections}
        ...
        # 4. Assigner les noms des joueurs
        used_bucks, used_celtics = set(), set()
        self.player_identity_map = {}
        
        # 4a. D'abord avec le modèle joueur
        for fn, frame in enumerate(video_frames):
            results = self.team_model(frame, verbose=False)
            if results[0].boxes:
                for box in results[0].boxes:
                        ...
                        if roi.size > 0:
                            ...
                            if r[0].boxes:
                                ...
                                if name and name in self.player_info:
                                    ...
                                    # Vérifier que le joueur correspond à l'équipe détectée
                                    if (real_team == "Milwaukee" and team == 1) or (real_team == "Celtics" and team == 2):
                                        if team == 1 and len(used_bucks) < self.max_players:
                                            self.player_identity_map[best_tid] = {"name": name, "team": team}
                                            used_bucks.add(name)
                                            print(f"  ✅ Track {best_tid} -> {name} (Bucks) [{len(used_bucks)}/{self.max_players}]")
                                        elif team == 2 and len(used_celtics) < self.max_players:
                                            self.player_identity_map[best_tid] = {"name": name, "team": team}
                                            used_celtics.add(name)
                                            print(f"  ✅ Track {best_tid} -> {name} (Celtics) [{len(used_celtics)}/{self.max_players}]")
        
        # 4b. Assignation aléatoire pour les autres (basée sur track_team)
        bucks_left = [p for p in self.bucks_players if p not in used_bucks]
        celtics_left = [p for p in self.celtics_players if p not in used_celtics]
